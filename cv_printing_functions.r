...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$idioms  <- read_gsheet(sheet_id = "idioms")
    ...
    cv$idioms  <-readr::read_csv(paste0(data_location, "idioms.csv"), skip = 1)
...
#' @param out_of The relative maximum for skills. Used to set what a fully filled in skill bar is.
#' 
print_idiom_bars <- function(cv, out_of = 5, bar_color = "#969696", bar_background = "#d9d9d9", glue_template = "default"){
